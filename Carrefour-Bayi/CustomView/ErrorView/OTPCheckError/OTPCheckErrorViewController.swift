//
//  OTPCheckErrorViewController.swift
//  Carrefour-Bayi
//
...
protocol OTPCheckErrorViewControllerDelegate: BaseViewModelDelegate {
    func resendCode()
    func closeButtonTapped()
}

class OTPCheckErrorViewController: BaseViewController {
    // MARK: -Properties
    var viewModel: OTPCheckErrorViewModel!
    weak var delegate: OTPCheckErrorViewControllerDelegate?
    ...
    @IBOutlet weak var tryAgainButton: BaseButton!
    @IBOutlet weak var resendCodeButton: BaseButton!
    @IBOutlet weak var titleTextLabel: UILabel!
    
    var timerValue = 120
    var isResendButtonActive = false
    var titleText = ""
    ...
    private func configureButtonAlphas() {
        if isResendButtonActive {
            resendCodeButton.alpha = 1
            resendCodeButton.isUserInteractionEnabled = true
            tryAgainButton.alpha = 0.5
    ...
    private func setUpProgressView() {
        ...
        progressView.start(beginingValue: timerValue, interval: 1.0)
    }
...
extension OTPCheckErrorViewController: TimerHandleDelegate {
...
// MARK: -OTPCheckErrorViewModelDelegate Methods
extension OTPCheckErrorViewController: OTPCheckErrorViewModelDelegate {
    func reSendCode() {
...
// MARK: - Creator
extension OTPCheckErrorViewController: XibNameProvider {
    static var xibName: String {
        get {
            return "OTPCheckErrorViewController"
        }
    }
    
    class func create(timerValue: Int, isResendButtonActive: Bool) -> OTPCheckErrorViewController {
        let vc: OTPCheckErrorViewController = OTPCheckErrorViewController.instantiateFromNib()
        let viewModel: OTPCheckErrorViewModel = OTPCheckErrorViewModel()
        vc.viewModel = viewModel
        vc.modalPresentationStyle = .overFullScreen
        vc.modalTransitionStyle = .crossDissolve
        vc.timerValue = timerValue
        vc.isResendButtonActive = isResendButtonActive
